...
ap.add_argument(
    "--out_dir", default=None, help="default: <filelist dir>/feature_prematched_<pool>"
)
...
ap.add_argument(
    "--cpu", action="store_true", help="force CPU even if faiss-gpu is present"
)
args = ap.parse_args()
...
if not args.cpu:
    try:
        ...
        print(
            f"FAISS: GPU not available -> CPU x{os.cpu_count()} threads "
...
print(
    f"{len(rows)} utterances, {len(by_sid)} speakers, "
...
def save(sid, i, pm):
    ...
    if BAR:
        ...
        if _done % _step == 0 or _done == N_TOTAL:
            ...
            print(
                f"  {_done}/{N_TOTAL} ({100 * _done // max(N_TOTAL, 1)}%) "
                f"sid={sid} {rate:.0f} utt/s ETA {int(eta)}s",
                flush=True,
            )
...
for sid, idxs in by_sid.items():
    ...
    if args.pool == "other":
        ...
        for i in present:
            ...
            d = ((feats[i][:, None, :] - pool[I]) ** 2).sum(
                -1
            )  # EXACT L2 on candidates [T, kk]
        ...
        kk = min(
            args.k + 64, len(allf)
        )  # coarse candidates, then EXACT re-rank + self-drop
        for i in order:
            _, I = index.search(feats[i], kk)  # faiss coarse: [T, kk] cand idx
            d = ((feats[i][:, None, :] - allf[I]) ** 2).sum(
                -1
            )  # EXACT L2 on candidates [T, kk]
...
print(f"\nwrote {out_fl}\n      {out_dir}/")
print(
    f"{len(new_lines)} prematched + {len(passthrough)} mute rows = "
    f"{len(new_lines) + len(passthrough)} total"
)
print(
    "Next: assemble a NEW arm folder with setup_prematch_arm.sh "
    f"(ORIG=<this exp>, POOL={args.pool}) -- it installs this filelist as the "
    "arm's filelist.txt. NO train.py changes needed."
)
