...
AUX_SPK = os.environ.get("AUX", "1") == "1"
AUX_CONTENT = os.environ.get("CONTENT", "1") == "1"
tau_spk, c_spk, c_content = 0.1, 3.0, 1.0

_scl_encoder = _scl_targets = _cenc = None
...
def _get_memory(device):
    ...
    if _membank is None:
        ...
        _membank = torch.from_numpy(ix.reconstruct_n(0, ix.ntotal)).float().to(device)
        print(f"[v2] memory bank: {p[0]} shape={tuple(_membank.shape)}", flush=True)
    return _membank


def _get_scl(device):
    """ECAPA speaker judge (frozen) + per-speaker centroids (spk_targets.pt)."""
    ...
    if _scl_encoder is None:
        ...
        for p in _scl_encoder.mods.parameters():
            p.requires_grad = False
        _scl_targets = torch.load(
            os.path.join(experiment_dir, "spk_targets.pt"), map_location=device
        ).to(device)
        print(f"AUX: loaded speaker targets {tuple(_scl_targets.shape)}", flush=True)
    return _scl_encoder, _scl_targets


def _get_content(device):
    """ContentVec judge (frozen), loaded from the LOCAL embedder folder."""
    global _cenc
    if _cenc is None:
...
lam_frag = float(os.environ.get("LAM_FRAG", "4.0"))
phon_every = int(os.environ.get("PHON_EVERY", "1"))
_phon_model = _phon_frag_ids = None


def _get_phon(device):
    """Frozen native Hindi CTC (judge-1) as a phonetic-percept reference."""
...
def main():
    ...
    def start():
        ...
        bad = [c.exitcode for c in children if c.exitcode not in (0, None)]
        if bad:
            print(
                f"TRAINING FAILED: worker exited with code {bad[0]} -- see traceback above."
            )
            sys.exit(1)
...
def train_and_evaluate(
    ...
    with tqdm(total=len(train_loader), leave=False) as pbar:
        for batch_idx, info in data_iterator:
            ...
            # (1) contrastive speaker loss: "who is speaking?" lineup
            if AUX_SPK and os.path.exists(
                ...
                assert int(sid.max()) < logits.shape[1], (
                    f"sid {int(sid.max())} but only {logits.shape[1]} centroids "
                    "-- regenerate spk_targets.pt"
                )
                loss_spk = F.cross_entropy(logits, sid) * c_spk
            else:
                loss_spk = torch.tensor(0.0, device=device)

            # (2) content loss (FIXED): the words of the REAL audio must
            # survive. Both sides encoded by ContentVec (independent judge)
            # -> valid for ANY input-feature space (contentvec / conformer /
            # prematched).
            if AUX_CONTENT and ids_slice is not None:
                cenc = _get_content(device)
                gen_cont = cenc(y16)["last_hidden_state"]
                with torch.no_grad():
                    tgt16 = torchaudio.functional.resample(
            ...
            # (3) phonetic content loss: same PHONEMES in, same phonemes out.
            # Probe-driven (Sept 6): features carry the tap contrast; the
            # generator drops it -> match the frozen judge's frame posteriors
            # on real vs generated audio, upweighting fragile characters.
            if os.environ.get("PHON_JUDGE") and global_step % phon_every == 0:
                pm, frag_ids = _get_phon(device)
    ...
    if rank == 0:
        ...
        if epoch % save_every_epoch == 0:
            try:
                ...
                # the bundled reference is ContentVec 768-d; a 512-d model cannot render it
                print(f"reference preview skipped ({e}) -- training continues")
                summarize(
                    writer=writer,
                    global_step=global_step,
                    images=image_dict,
