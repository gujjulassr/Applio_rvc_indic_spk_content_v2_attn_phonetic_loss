...
class VoiceConverter:
    ...
    def setup_network(self):
        ...
        if self.cpt is not None:
            ...
            # derive the input-feature dim from the checkpoint itself instead of
            # hardcoding by version: v1=256, v2 ContentVec=768, IndicConformer
            # native=512. emb_phone is Linear(embedding_dim, hidden), so its
            # weight is [hidden, embedding_dim] -> shape[1] is the feature dim.
            if "enc_p.emb_phone.weight" in self.cpt["weight"]:
                self.text_enc_hidden_dim = self.cpt["weight"][
